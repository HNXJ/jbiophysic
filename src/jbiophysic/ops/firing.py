...
def firing_rate(spikes: jnp.ndarray, dt_ms: float) -> float:
    """Compute the population mean firing rate (Hz)."""
    return float(jnp.mean(per_neuron_firing_rate(spikes, dt_ms)))

def max_single_neuron_rate(spikes: jnp.ndarray, dt_ms: float) -> float:
    """Compute the maximum firing rate (Hz) across all neurons."""
    return float(jnp.max(per_neuron_firing_rate(spikes, dt_ms)))
